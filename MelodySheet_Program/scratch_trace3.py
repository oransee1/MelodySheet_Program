...
def recursive_split(a, b, active_array, merge_gap_for_runs, max_sys_h):
    if b - a <= max_sys_h:
        return [(a, b)]
    ...
    return recursive_split(split_a, split_b, active_array, merge_gap_for_runs, max_sys_h) + \
           recursive_split(split_c, split_d, active_array, merge_gap_for_runs, max_sys_h)
...
for i in range(len(page_y_positions) - 1):
    ...
    for a, b in regions:
        if b - a > max_sys_h:
            ...
            pieces = recursive_split(a, b, active, int(dpi * 0.15), max_sys_h)
            split_regions.extend(pieces)
            for p in pieces:
                print(f"  -> Added {p} (height {p[1]-p[0]})")
        else:
    ...
    print("Filtered regions:", filtered)
